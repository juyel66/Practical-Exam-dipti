...
const TownViewProperty = () => {
    const townViews = useLoaderData();
    const {id1} = useParams();
    const idIn = parseInt(id1)


    // console.log(id1,townViews)
    const townView = townViews.find(townView => townView.id1 ===idIn)
    // console.log(townView)
    const { image, estate_title, description, price, area, facilities, location, status, segment_name }= townView;
    
    return (
        <div data-aos="zoom-in">
            <Helmet><title>Townhouses Details</title></Helmet>
            <div>
...
export default TownViewProperty;